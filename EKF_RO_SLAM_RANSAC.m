function [x_new_new, P] = EKF_RO_SLAM_RANSAC(x,P,z,u,idx, R, Q)  
    % EKF Range-Only SLAM
...
function [x_measure,H] = h(x,idx)
    ...
    % Jacobian of h
    H = zeros(1,length(x));
    H(1) = (x(1) - lmx)/sqrt((x(1) - lmx)^2 + (x(2) - lmy)^2);
    H(2) = (x(2) - lmy)/sqrt((x(1) - lmx)^2 + (x(2) - lmy)^2);
    H((idx-1)*2 + 4) = -1*(x(1) - lmx)/sqrt((x(1) - lmx)^2 + (x(2) - lmy)^2);
    H((idx-1)*2 + 5) = -1*(x(2) - lmy)/sqrt((x(1) - lmx)^2 + (x(2) - lmy)^2);
end